...
metadata <- readxl::read_excel('example_data/Example of difference in plate layout/2025_1_7_ABazidedosecurve_50_75_plate1.xlsx',
                               skip = 1) %>%
...
zebrabox_data %>%
  filter(datatype == 'QuantizationSum', !is.na(activity),
         treatment == control_group, light == 'dark') %>%
...
zebrabox_data %>%
  filter(datatype == 'QuantizationSum', !is.na(activity)) %>%
  group_by(plate, treatment, light, period2, well) %>%
  mutate(index = row_number()) %>% #ungroup() %>% filter(well == 'A01', plate == 'LocA') %>% select(plate, condition, light, time_min, period2, pn, well, index, activity)
  nest() %>% 
  ungroup() %>%
  mutate(auc = map(data, ~ try(as_tibble(pracma::cumtrapz(x = .$time_min, y = .$activity)))),
         class = map(auc, ~ class(.))) %>% 
  unnest(c(class)) %>%
  filter(class != 'try-error') %>%
  select(-class) %>%
  unnest(c(auc)) -> zebrabox_auc_all_fish

zebrabox_auc_all_fish %>%
  group_by(treatment, light, well) %>%
  summarize(mean_auc = mean(V1)) %>%
  ungroup() %>%
  filter(light == 'dark') %>%
ggplot(aes(x = treatment, y = mean_auc)) +
  ggbeeswarm::geom_quasirandom() +
  geom_boxplot(alpha = 0) +
  labs(x = 'Condition', 
       y = 'Mean Area Under the Curve\nAcross All Dark Cycles',
       title = 'Zebrabox') +
  theme_bw()
```
...
```{r}
zebrabox_auc_all_fish %>%
  filter(treatment == control_group, light == 'dark') %>%
  summarize(median(V1)) %>%
  deframe() -> median_auc
  

zebrabox_auc_all_fish %>%
  mutate(percent_change = ((V1 - median_auc) / median_auc) * 100) %>%
  group_by(treatment, light, well) %>%
  summarize(mean_activity = mean(percent_change)) %>%
  ungroup() %>%
  filter(light == 'dark') %>%
ggplot(aes(x = treatment, y = mean_activity)) +
  ...
  geom_boxplot(alpha = 0) +
  labs(x = 'Condition', 
       y = 'Average Percent Change From Control Median AUC\nin Dark Cycles',
       title = 'Percent Change AUC') +
  theme_bw()
```
...
zebrabox_data %>%
  ...
  ungroup() %>%
  filter(light == 'dark') %>%
  aov(mean_activity ~ treatment, data = .) %>%
  TukeyHSD(.) %>%
  broom::tidy(.) %>%
  select(groups_tested= contrast,
